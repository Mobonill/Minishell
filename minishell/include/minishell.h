...
# define STX_ERR "bash: syntax error near unexpected token "
# define RESET "\033[0m"
...
void			expand_command_args(char **str_array, t_shell *shell);
...
// EXECUTE CMDS
void			ft_cmd_path(char *cmd_path, t_simple_cmds *parser,
					t_exec *exec);
void			execute_command(t_simple_cmds *parser, t_shell *shell,
					t_exec *exec);
...
int				handle_redirections_and_heredoc(t_exec *exec,
					t_simple_cmds *parser, int i);
int				handle_redirections(t_exec *exec, t_simple_cmds *parser,
					t_lexer *redir);
int				handle_heredoc_redirection(t_exec *exec,
					t_simple_cmds *parser, t_lexer *redir);
...
int				init_and_allocate(t_exec *exec,
					t_simple_cmds *parser, t_shell *shell);
